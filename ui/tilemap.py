    ...
    def load_tiles(self,text_map,materials,offset):
        ...
        for x in range(0,self.screen_width,self.tile_size): 
            ...
            for y in range(0,self.screen_height,self.tile_size):
                ...
                if text.find(":") == -1:
                    row.append(Tile(Vector(x,y) + offset,materials[text_map[x//self.tile_size][y//self.tile_size]]))
                else:
                    material,frame = text.split(":")
                    row.append(Tile(Vector(x,y) + offset,materials[material],int(frame),self.tile_size))
            tiles.append(row)
        return tiles
